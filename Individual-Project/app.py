...
@app.route('/DONOTOPEN', methods=['GET', 'POST'])
def add_data():
...
def home():
    if request.method == "POST":
        ...
        try:
            ...
            UID = login_session["user"]["localId"]
            templ_comments = {"UID": UID, "story": story}
            db.child("Countries").child(country).child("comments").push(templ_comments)
            comments = db.child("Countries").child(country).child("comments").get().val()
            return redirect(url_for('home'))
        except:
        ...
        if login_session['user'] == None:
            ...
            return render_template("home.html", countries = countries, users = users)
